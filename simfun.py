...
def setup(input_file):
    ...
    #Check if PSF has correct size, interpolate if not. psf_col must be specified in that case
    import warnings
    with warnings.catch_warnings(): #This is to suppress the potential "FutureWarning" error message. Comparing np-array to str etc. Might cause errors down the line?
        ...
        if not psf.shape[2] == span:
            if psf_col == ''  :
                raise TypeError("psf_col must be specified, since psf is incomplete in the wl_ran")
            if not psf_col.shape[0] == psf.shape[2]:
                raise TypeError("psf_col must be the same length as no. of slices in psf")
            else:
                psf = psf_interp(input_psf_images=psf, input_psf_wl = psf_col, wl_endpoints=wl_ran) #Make new PSF, with interpolated values
    del psf_col
    ...
    with warnings.catch_warnings(): #This is to suppress the potential "FutureWarning" error message. Comparing np-array to str etc. Might cause errors down the line?
        ...
        if jitter == '': #if jitter is an empty str, it will also be generated.
            ...
            spec_eff = spec_eff/step #If the generated jitter is used, the spectrum must be in step size, not seconds
            spec_eff2 = spec_eff2/step
    jitter = jitter_im(x= jitter[:,0], y= jitter[:,1], psf_size=(psf[:,:,0].shape[0], psf[:,:,0].shape[0]) )

    return spec_eff, spec_eff2, jitter, x_j, y_j, psf, img_size, sub_pixel, pl_arc_mm, disper, mask, slitpos, background
